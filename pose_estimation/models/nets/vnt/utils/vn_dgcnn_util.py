...
def get_graph_feature_cross(x, k=20, use_global=False, idx=None, device='cpu'):
    if use_global:
        return get_graph_feature_cross_global(x, k, idx, device=device)
    else:
        return get_graph_feature_cross_local(x, k, idx, device=device)


def get_graph_feature_cross_global(x, k=20, idx=None, device='cpu'):
    batch_size = x.size(0)
    num_points = x.size(3)
    #To preserve translation
    bias = torch.mean(x, dim=-1, keepdim=True)
...
def get_graph_feature_cross_local(x, k=20, idx=None, device='cpu'):
    batch_size = x.size(0)
    num_points = x.size(3)
    x = x.view(batch_size, -1, num_points)
    if idx is None:
